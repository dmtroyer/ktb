...
ktb.controller('PanelListCtrl', ['$scope', 'Panels', function($scope, Panels) {
  $scope.panels = Panels.query(function() {
    $scope.panels.installed_panel_set.map(function(panel) {
      delete panel.pnl_pk;
      panel.created_by = stripArtists(panel.created_by);
...
ktb.controller('PanelDetailCtrl', ['$scope', '$routeParams', 'Panel', function($scope, $routeParams, Panel) {
...
ktb.controller('AppCtrl', ['$scope', '$location', function($scope, $location) {
...
function stripArtists(shite) {
  var matches = /<strong>([\w\s]*)<\/strong>/g,
    artists = [],
    item;
  while (item = matches.exec(shite))
  {
    if (artists.indexOf(item[1]) == -1) { artists.push(item[1]); }
  }
  return artists;
}
